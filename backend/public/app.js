...
function renderOrders(ordersList) {
  ...
  ordersList.forEach(ord => {
    ...
    // Status text mapping
    let statusText = ord.orderStatus;
    let badgeClass = 'badge-success';
    if (ord.orderStatus === 'pending') { badgeClass = 'badge-danger'; statusText = 'Pending'; }
    else if (ord.orderStatus === 'accepted') { badgeClass = 'badge-success'; statusText = 'Accepted'; }
    else if (ord.orderStatus === 'packed') { badgeClass = 'badge-success'; statusText = 'Packed'; }
    else if (ord.orderStatus === 'out_for_delivery') { badgeClass = 'badge-success'; statusText = 'Dispatched'; }

    // Action button mapping
    let actionBtn = '';
    if (ord.orderStatus === 'pending') {
      actionBtn = `<button class="btn btn-primary w-100" onclick="updateStatus('${ord._id}', 'accepted')">Accept Order</button>`;
    } else if (ord.orderStatus === 'accepted') {
      actionBtn = `<button class="btn btn-primary w-100" style="background:var(--purple); box-shadow:none;" onclick="updateStatus('${ord._id}', 'packed')">Mark Packed</button>`;
    } else if (ord.orderStatus === 'packed') {
      actionBtn = `<button class="btn btn-primary w-100" style="background:var(--orange); box-shadow:none;" onclick="updateStatus('${ord._id}', 'out_for_delivery')">Dispatch Order</button>`;
    } else if (ord.orderStatus === 'out_for_delivery') {
      actionBtn = `<button class="btn btn-primary w-100" style="background:var(--green); box-shadow:none;" onclick="updateStatus('${ord._id}', 'delivered')">Mark Delivered</button>`;
    }
    ...
    card.innerHTML = `
      <div class="order-header">
        ...
        <div class="order-meta-info">
          ...
          <span style="font-weight: 700; font-size: 1.15rem; color: var(--accent-color);">Total: ₹${ord.totalAmount}</span>
        </div>
      </div>
      <div class="order-body-grid">
        <div class="customer-details">
          <h4>Customer</h4>
          <p><strong>${ord.customerId?.name || 'Walk-in Customer'}</strong></p>
          <p><i data-lucide="phone" style="width:14px;"></i> ${ord.customerId?.phone || 'No phone'}</p>
        </div>
          ...
          <div class="ordered-items-list">
            ${ord.products.map(p => `
              <div class="order-item-row">
                <span>${p.productId?.productName || 'Deleted Product'} (${p.quantity} ${p.productId?.unit || 'qty'})</span>
